...
static uint16_t press_char_handle;
static uint16_t hum_char_handle;

#define ENVIRONMENTAL_SENSING_SERVICE_UUID 0x181a
#define TEMPERATURE_CHAR_UUID 0x2a6e
#define PRESSURE_CHAR_UUID 0x2a6d
...
static void Sensors_Init() {
  ...
  hts221_io.GetTick = BSP_GetTick;
  hts221_io.Address = HTS221_I2C_ADDRESS;

  HTS221_RegisterBusIO(&hts221_obj, &hts221_io);
  HTS221_Init(&hts221_obj);
  HTS221_TEMP_SetOutputDataRate(&hts221_obj, HTS221_ODR_1Hz);
  HTS221_HUM_SetOutputDataRate(&hts221_obj, HTS221_ODR_1Hz);
  ...
  lps22hb_io.Init = BSP_I2C2_Init;
  lps22hb_io.DeInit = BSP_I2C2_DeInit;
  lps22hb_io.WriteReg = BSP_I2C2_WriteReg;
  lps22hb_io.ReadReg = BSP_I2C2_ReadReg;
  lps22hb_io.GetTick = BSP_GetTick;
  lps22hb_io.Address = LPS22HB_I2C_ADD_H;

  LPS22HB_RegisterBusIO(&lps22hb_obj, &lps22hb_io);
  LPS22HB_Init(&lps22hb_obj);
  LPS22HB_PRESS_SetOutputDataRate(&lps22hb_obj, LPS22HB_ODR_1_Hz);
  LPS22HB_PRESS_Enable(&lps22hb_obj);
}

static void BLE_Init() {
  const char deviceName[] = "ArmIotLab7";

  const char localName[] = {AD_TYPE_COMPLETE_LOCAL_NAME,
                            'A',
                            'r',
                            'm',
                            'I',
  ...
  aci_gap_init_IDB05A1(GAP_PERIPHERAL_ROLE_IDB05A1, 0, 9, &gap_service_handle,
                       &dev_name_char_handle, &appearance_char_handle);

  aci_gatt_update_char_value(gap_service_handle, dev_name_char_handle, 0,
                             sizeof(deviceName) - 1, deviceName);

  aci_gap_set_discoverable(ADV_IND, 0x20, 0x40, PUBLIC_ADDR, NO_WHITE_LIST_USE,
                           sizeof(localName), localName, 0, NULL, 0, 0);

  aci_hal_set_tx_power_level(1, 4);
}

static void BLE_Sensor_Init() {
  uint16_t service_uuid = ENVIRONMENTAL_SENSING_SERVICE_UUID;
  uint16_t temp_char_uuid = TEMPERATURE_CHAR_UUID;
  uint16_t press_char_uuid = PRESSURE_CHAR_UUID;
  uint16_t hum_char_uuid = HUMIDITY_CHAR_UUID;

  aci_gatt_add_serv(UUID_TYPE_16, (const uint8_t *)&service_uuid,
                    PRIMARY_SERVICE, 16, &env_service_handle);

  aci_gatt_add_char(env_service_handle, UUID_TYPE_16,
                    (const uint8_t *)&temp_char_uuid, 2,
                    CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE,
                    GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 0, &temp_char_handle);

  aci_gatt_add_char(env_service_handle, UUID_TYPE_16,
                    (const uint8_t *)&press_char_uuid, 4,
                    CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE,
                    GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 0, &press_char_handle);

  aci_gatt_add_char(env_service_handle, UUID_TYPE_16,
                    (const uint8_t *)&hum_char_uuid, 2,
                    CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE,
                    GATT_NOTIFY_ATTRIBUTE_WRITE, 16, 0, &hum_char_handle);
}

static void BLE_Sensor_Update() {
  float temperature = 0;
  HTS221_TEMP_GetTemperature(&hts221_obj, &temperature);

  float humidity = 0;
  HTS221_HUM_GetHumidity(&hts221_obj, &humidity);

  float pressure = 0;
  LPS22HB_PRESS_GetPressure(&lps22hb_obj, &pressure);

  int16_t temp_val = (temperature / 0.01);
  uint32_t pres_val = pressure * 10;
  uint16_t hum_val = humidity * 100;

  aci_gatt_update_char_value(env_service_handle, temp_char_handle, 0,
                             sizeof(temp_val), (uint8_t *)&temp_val);

  aci_gatt_update_char_value(env_service_handle, press_char_handle, 0,
                             sizeof(pres_val), (uint8_t *)&pres_val);

  aci_gatt_update_char_value(env_service_handle, hum_char_handle, 0,
                             sizeof(hum_val), (uint8_t *)&hum_val);
}
...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOA,
                    ARD_D10_Pin | GPIO_PIN_5 | SPBTLE_RF_RST_Pin | ARD_D9_Pin,
                    GPIO_PIN_RESET);
