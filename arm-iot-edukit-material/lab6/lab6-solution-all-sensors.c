...
int main(void) {
  ...
  char buffer[128] = {0};
  sprintf(buffer, "--- Sensors ---\n");
  HAL_UART_Transmit(&huart1, (const uint8_t *)buffer, strlen(buffer), 10);
  ...
  while (1) {
    ...
    // --- LSM6DSL --- //
    LSM6DSL_Axes_t axes;
    LSM6DSL_ACC_GetAxes(&lsm6dsl_obj, &axes);

    sprintf(buffer, "Accelerometer: %d %d %d\n", axes.x, axes.y, axes.z);
    HAL_UART_Transmit(&huart1, (const uint8_t *)buffer, strlen(buffer), 10);

    LSM6DSL_GYRO_GetAxes(&lsm6dsl_obj, &axes);
